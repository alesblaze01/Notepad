...
public class DatabaseDataWorker {
    ...
    private void insertCourse(String courseId, String title) {
        ...
        values.put(NotesDatabaseContract.CourseInfoEntry.COLUMN_COURSE_ID, courseId);
        values.put(NotesDatabaseContract.CourseInfoEntry.COLUMN_COURSE_TITLE, title);

        long newRowId = mDb.insert(NotesDatabaseContract.CourseInfoEntry.TABLE_NAME, null, values);
    }

     private void insertNote(String courseId, String title, String text) {
        ContentValues values = new ContentValues();
        values.put(NotesDatabaseContract.NotesInfoEntry.COLUMN_COURSE_ID, courseId);
        values.put(NotesDatabaseContract.NotesInfoEntry.COLUMN_NOTE_TITLE, title);
        values.put(NotesDatabaseContract.NotesInfoEntry.COLUMN_NOTE_TEXT, text);

        long newRowId = mDb.insert(NotesDatabaseContract.NotesInfoEntry.TABLE_NAME, null, values);
    }
